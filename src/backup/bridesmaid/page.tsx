...
function getProductDetailHref(
  ...
  return `/${cleanCategoryPath}/products/${encodeURIComponent(productSlugOrId)}`;
}
...
export function BridesmaidPageContent({
  categoryPath,
}: {
  categoryPath: string;
}) {
      ...
      fallback={
        <main className="flex min-h-screen items-center justify-center bg-[#fbf8f1] text-[#15100c]">
          <p className="text-sm text-[#6d6760]">
            Loading bridesmaid products...
          </p>
        </main>
      }
    >
   <BridesmaidPageContent categoryPath="bridesmaid" />
    </Suspense>
  );
}
